...
- `cd` into the directory you just cloned down
...
You're provided a basic website with included `index.html`, `style.css`, and `script.js`. 

All of your work will go in `script.js` - you shouldn't have to edit `index.html` or `style.css`. 
...
- Be specific when internet searching information you might need to solve a problem (eg: add "js" to the end if searching a JavaScript problem)
...
DOM's personal website title is a bit wordy. Write a JavaScript statement that selects the `#main-title` ID element. Remember there are a couple of ways to query **id**. 
...
QUESTION: How can we always select the *last* item without hardcoding in the index number? There are a couple different ways...
...
![Wooo](https://i.imgur.com/Nhy7EON.gif?noredirect)
...
- PR must be submitted utilizing these guidelines: [PR Guidelines](https://github.com/SEI-08-Bahrain/pr_template)
...
![Goodbye](https://thumbs.gfycat.com/AccomplishedImpoliteDuckling-size_restricted.gif)
